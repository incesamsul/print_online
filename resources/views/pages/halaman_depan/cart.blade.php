        ...
        <div class="row">
            <div class="col-sm-8">
                <div class="section-title table-responsive">
                        ...
                        @if (count($cart) > 0)
                            @foreach ($cart as $id=>$row)
                                <tr>
                                    ...
                                    <td class="align-middle">{{ $row->lbr_warna }}</td>
                                    <td class="align-middle">{{ $row->lbr_bw }}</td>
                                    <td class="align-middle">{{ count_pdf_pages($row->file) }}</td>
                                    <td class="align-middle">Rp. {{ count($cart) > 0 ? number_format(($row->produk->harga_warna * $row->lbr_warna) + ($row->produk->harga_bw * $row->lbr_bw) )  : '' }}</td>
                    ...
                    @if (count($cart) > 0)
                    <a href="{{ URL::to('/hitung_total_pembayaran/' . $row->file . '/' . $row->id_cart) }}" class="btn-hitung btn bg-main text-white"><i class="fa fa-plus"></i> Hitung total Pembayaran</a>
                    @endif
                </div>
            </div>
            <div class="col-sm-4">
                <div class="section-title">
                    ...
                    <table class="table">
                        ...
                        <tr>
                            <th>Sub total</th>
                            <th>Rp. {{ count($cart) > 0 ? number_format(($row->produk->harga_warna * $row->lbr_warna) + ($row->produk->harga_bw * $row->lbr_bw) )  : '' }}</th>
                        </tr>
                    </table>
                    @if (count($cart) > 0 && $row->lbr_warna > 0 && $row->lbr_bw > 0)
                            ...
                            <div class="col-sm-6 my-2">
                                <div class="card border-0 hover-card">
                                    <form action="{{ URL::to('/transaksi') }}" method="POST">
                                        ...
                                        <input id="total_pembayaran" type="hidden" name="total_pembayaran" value="{{ count($cart) > 0 ? ($row->produk->harga_warna * $row->lbr_warna) + ($row->produk->harga_bw * $row->lbr_bw)   : '' }}">
